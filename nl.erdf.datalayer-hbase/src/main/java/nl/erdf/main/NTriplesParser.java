...
public class NTriplesParser extends RDFParserBase {
	// Logger
	protected static Logger logger = LoggerFactory.getLogger(NTriplesParser.class);
	 ...
	 */
	public synchronized void parse(InputStream in, String baseURI) throws IOException, RDFParseException,
			RDFHandlerException {
		if (in == null) {
	 ...
	 */
	public synchronized void parse(Reader reader, String baseURI) throws IOException, RDFParseException,
			RDFHandlerException {
		if (reader == null) {
		...
		try {
			...
			while (c != -1) {
				if (c == '#') {
					...
					// HACK from original version not to break on errors
					try {
						c = parseTriple(c);
					} catch (org.openrdf.rio.RDFParseException e) {
						// Ignore the triple and consume the end of the line
