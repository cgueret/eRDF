 ...
 * 
 */
public class SPARQLStmtExtractor implements QueryModelVisitor<Exception> {
	// Logger instance
	private final static Logger logger = LoggerFactory.getLogger(SPARQLStmtExtractor.class);

	// Set of patterns extracted from the query
	private final Set<StatementPattern> patterns = new HashSet<StatementPattern>();

	/**
	 * 
	 */
	protected SPARQLStmtExtractor() {
	}

	/**
	 * @param fileName
	 */
	public static Set<StatementPattern> parseFromFile(File fileName) {
		logger.info("Parsing query " + fileName.getAbsolutePath());
		return parseFromString(FileToText.convert(fileName));
	}

	/**
	 * @param queryString
	 */
	public static Set<StatementPattern> parseFromString(String queryString) {
		SPARQLStmtExtractor instance = new SPARQLStmtExtractor();
		SPARQLParser parser = new SPARQLParser();
		try {
			ParsedQuery query = parser.parseQuery(queryString, null);
			query.getTupleExpr().visitChildren(instance);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void meet(StatementPattern statementPattern) throws Exception {
		// Reject fully instantiated statements
